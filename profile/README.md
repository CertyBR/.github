<p align="center">
  <img src="https://certy.zerocert.com.br/logo.svg" alt="Certy" width="180" />
</p>

# Certy

O **Certy** é um projeto para simplificar a emissão de certificados TLS/SSL e tornar a adoção de HTTPS mais acessível para times e pessoas desenvolvedoras.

Nosso foco é transformar um processo sensível e cheio de etapas em uma experiência clara, segura e confiável.

## Por que essas tecnologias

- **SvelteKit + TypeScript (Frontend):** entrega uma experiência rápida e moderna para quem está emitindo e acompanhando certificados.
- **Cloudflare Workers (Proxy):** adiciona uma camada de proteção e controle entre cliente e backend com baixa latência global.
- **Rust + Axum (Backend):** garante alta confiabilidade e desempenho para o fluxo crítico de emissão.
- **PostgreSQL (Dados):** persistência sólida para sessões, auditoria e estado operacional do sistema.

## Estatísticas

<p align="center">
  <img src="https://img.shields.io/github/stars/CertyBR/certy.com.br?style=for-the-badge&logo=github&label=Stars" alt="stars" />
  <img src="https://img.shields.io/github/forks/CertyBR/certy.com.br?style=for-the-badge&logo=github&label=Forks" alt="forks" />
  <img src="https://img.shields.io/github/issues/CertyBR/certy.com.br?style=for-the-badge&logo=github&label=Issues" alt="issues" />
</p>

<p align="center">
  <img src="https://img.shields.io/github/last-commit/CertyBR/certy.com.br?style=for-the-badge&logo=git&label=Last%20Commit" alt="last commit" />
  <img src="https://img.shields.io/github/license/CertyBR/certy.com.br?style=for-the-badge&label=License" alt="license" />
  <img src="https://img.shields.io/github/followers/CertyBR?style=for-the-badge&logo=github&label=Followers" alt="followers" />
</p>

## Links

- Site: https://certy.com.br
- Projeto: https://github.com/CertyBR/certy.com.br
- Organização: https://github.com/CertyBR
- Patrocínio: https://zerocert.com.br

## Mantenedores

- André Ribas — https://github.com/RibasSu
- Sarah Maia — https://github.com/sarahsec
